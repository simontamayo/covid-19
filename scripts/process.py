...
def pivot_key_countries(package):
    ...
    for country in key_countries:
        package.pkg.descriptor["resources"][1]["schema"]["fields"].append(
            dict(
                name=country,
                type="integer",
...
def calculate_increase_rate(package):
    ...
    def process_rows(rows):
        ...
        for row in rows:
            if previous_row:
                row["Increase rate"] = (
                    (row["Confirmed"] - previous_row["Confirmed"])
                    / previous_row["Confirmed"]
                    * 100
                )
            previous_row = row
            yield row
...
def fix_canada_recovered_data(rows):
    ...
    for row in rows:
        if (
            row.get("Country/Region") == "Canada"
            and row.get("Province/State") == "Recovered"
            and not row.get("Recovered")
        ):
            continue
...
Flow(
    ...
    update_resource(
        "time_series_covid19_deaths_US", name="us_deaths", path="data/us_deaths.csv"
    ),
    update_schema(
        ...
        fields=[
            ...
            {"format": "default", "name": "Province/State", "type": "string"},
            {
                "decimalChar": ".",
                "format": "default",
                "groupChar": "",
    ...
    join_with_self(
        ...
        fields=dict(
            Date={"name": "Date"},
            Confirmed={"name": "Confirmed", "aggregate": "sum"},
            Recovered={"name": "Recovered", "aggregate": "sum"},
            Deaths={"name": "Deaths", "aggregate": "sum"},
        ),
    ),
    printer(),
    update_schema(
    ...
    update_schema(
        ...
        fields=[
            {"format": "%Y-%m-%d", "name": "Date", "type": "date"},
            {"format": "default", "name": "Country", "type": "string"},
            {
                "format": "default",
                "groupChar": "",
                "name": "Confirmed",
    ...
    update_package(
        ...
        views=[
            ...
            },
            {
                ...
                "specType": "simple",
                "spec": {"group": "Date", "series": ["Mortality rate"], "type": "bar"},
            },
            {
